...
func pushImageToIPFS(t *testing.T, base *testutil.Base, name string, opts ...string) string {
	...
	t.Logf("ctr container (BEFORE,nerdctl-test): %s", res.Combined())
	t.Logf("ps(BEFORE): %s", base.Cmd("ps", "-a").Run().Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "snapshot", "list"))
	t.Logf("ctr snapshot (BEFORE,nerdctl-test): %s", res.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "content", "list"))
	t.Logf("ctr content (BEFORE,nerdctl-test): %s", res.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "image", "list"))
	t.Logf("ctr image (BEFORE,nerdctl-test): %s", res.Combined())
	pullRes := base.Cmd("pull", name).Run()
	t.Logf("pull: %s", pullRes.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "container", "list"))
	t.Logf("ctr container (AFTER,nerdctl-test): %s", res.Combined())
	t.Logf("ps(BEFORE): %s", base.Cmd("ps", "-a").Run().Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "snapshot", "list"))
	t.Logf("ctr snapshot (AFTER,nerdctl-test): %s", res.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "content", "list"))
	t.Logf("ctr content (AFTER,nerdctl-test): %s", res.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "--namespace=nerdctl-test", "image", "list"))
	t.Logf("ctr image (AFTER,nerdctl-test): %s", res.Combined())
	res = icmd.RunCmd(icmd.Command("ctr", "content", "list"))
	t.Logf("ctr(default): %s", res.Combined())
